...
void instrument_function_if_needed(CUcontext ctx, CUfunction func) {
	...
	/*	for (auto i : instrs) {
		...
		open_profile_file(profileOut);
 		//fout2.open(profileOut.c_str(), std::ifstream::out);

		std::string kname = removeSpaces(nvbit_get_func_name(ctx,f));
		maxregs = -1;
		cuFuncGetAttribute(&maxregs, CU_FUNC_ATTRIBUTE_NUM_REGS, f);
		assert(fout2.good());
		//fout2<< "Inspecting: " << kname << ";num_static_instrs: " << instrs.size() << ";maxregs: " << maxregs << "(" << maxregs << ")" << std::endl;
	}
}
	...
	if (cbid == API_CUDA_cuLaunch ||
		...
		if(!is_exit) {
			...
			if ( cbid == API_CUDA_cuLaunchKernel_ptsz ||
				...
				num_threads = p2->blockDimX * p2->blockDimY * p2->blockDimZ; //added
			}
			assert(fout.good());
			std::string kname = removeSpaces(nvbit_get_func_name(ctx, p->f)).c_str();
			int nDevices = 1;			
			...
			fout2 << "NVBit-igprofile; index: " << kernel_id++ << "; kernel_name: " << kname ;
				
			fout2<< "; num_threads: " << num_threads << "; n_devices: "<< nDevices << "; maxregs: "<< maxregs<<  std::endl;
